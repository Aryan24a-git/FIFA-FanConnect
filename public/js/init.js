...
(function () {
  ...
  if (container) {
    ...
    for (let i = 0; i < origPos.count; i += 3) {
      ...
      hexagonCenters.push(
        new THREE.Vector3().addVectors(v1, v2).add(v3).divideScalar(3).normalize(),
      );
    }
    ...
    const palette = [
      ...
      new THREE.Color(0xff5e00),
      new THREE.Color(0x88929b),
    ...
    for (let i = 0; i < positionAttribute.count; i += 3) {
      ...
      for (let j = 0; j < pentagonCenters.length; j++) {
        if (
          v1.clone().normalize().distanceTo(pentagonCenters[j]) < 0.01 ||
      ...
      v1.lerp(panelCenter, 1 - shrinkFactor)
        .normalize()
        .multiplyScalar(radius);
      v2.lerp(panelCenter, 1 - shrinkFactor)
        .normalize()
        .multiplyScalar(radius);
      v3.lerp(panelCenter, 1 - shrinkFactor)
        .normalize()
        .multiplyScalar(radius);
      ...
      const pHash =
        Math.abs(
          Math.sin(panelCenter.x * 12.9898 + panelCenter.y * 78.233 + panelCenter.z * 37.719),
        ) * 43758.5453;
      ...
      if (isPatterned) {
        const tHash =
          Math.abs(Math.sin(center.x * 12.9898 + center.y * 78.233 + center.z * 37.719)) *
          43758.5453;
    ...
    const pl = new THREE.PointLight(0x00e5ff, 1.5, 50);
    pl.position.set(5, -5, 5);
    scene.add(pl);
    const bl = new THREE.PointLight(0xffd700, 1.5, 50);
